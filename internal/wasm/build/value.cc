...
bool Value::Metadata::is_valid() const {
  ...
  return content()->type()->kind() > 0;
}

Value::Metadata::Metadata(const Type* type, bool is_null,
                          bool preserves_order) {
  *content() = Content(const_cast<Type *>(type), is_null, preserves_order);
